    ...
    stages {
        stage('Environment Setup') {
            steps {
                ...
                script {
                    // Dynamically downloads a standalone lightweight Docker client command binary straight into the workspace
                    sh '''
                        if [ ! -f ./docker ]; then
        ...
        stage('Build') {
            steps {
                echo '=== STAGE 1: Production Artifact Building ===' [cite: 93, 114, 125]
                // Executes the build using our local workspace binary client wrapper
        ...
        stage('Test') {
            steps {
                echo '=== STAGE 2: Advanced Test Strategy (Unit + Integration) ===' [cite: 93, 116, 144]
        ...
        stage('Code Quality') {
            steps {
                echo '=== STAGE 3: Advanced Code Quality Gating ===' [cite: 93, 118]
        ...
        stage('Security') {
            steps {
                echo '=== STAGE 4: Proactive Security Vulnerability Scanning ===' [cite: 93, 119, 153]
        ...
        stage('Deployment') {
            steps {
                echo '=== STAGE 5: Zero-Downtime Automated Deployment ===' [cite: 93, 120, 158]
        ...
        stage('Release') {
            steps {
                echo '=== STAGE 6: Version-Controlled Environment Release ===' [cite: 93, 122, 166]
                sh """
        ...
        stage('Monitoring') {
            steps {
                echo '=== STAGE 7: Live Metrics Integration & Incident Simulation ===' [cite: 93, 123, 170]
                sh 'sleep 7'
                script {
